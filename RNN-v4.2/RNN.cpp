	...
	AOptimizer* opt, int n_threads, double lambda)
{
	...
	vector<future<map<string, mat>>> vec_future; // 存储多线程的vector, future的泛型是 方法返回值
	mat dWfSum, dbfSum;
	mat dWiSum, dbiSum;
	mat dWcSum, dbcSum;
	mat dWoSum, dboSum;
	...
	*/
	...
	for (int t = (int)inputs.n_rows - 1; t >= 0; t--)
	{
		if (t == inputs.n_rows - 1)
		{
			...
			dX = dXo + dXc + dXi + dXf; // X 中[h, x]

			// update dhnext dcnext
			dhnext = dX.cols(0, H-1); // dxnext 可以简单得到
			dcnext = h_fs[t] % dc;
