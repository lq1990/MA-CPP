...
void RNN::initParams()
{
	...
	this->Why = arma::randn(this->n_hidden, this->n_output_classes) * 0.01;
...
/*
	...
	matData is the inputs.
	score is the label.
	这个函数只针对一个场景。
	训练所有场景时，可以在循环体内调用它。
*/
void RNN::train(map<const char*, MyStruct> allScenarios)
{
	...
	*/
	for (int i = 0; i < this->totalSteps; i++)
	{
		...
		*/
		...
		for (ascenario = allScenarios.begin();
			ascenario != allScenarios.end(); 
			ascenario++)
		{
...
map<string, mat> RNN::lossFun(mat inputs, double score, mat hprev)
{
	...
	for (int t = 0; t < inputs.n_rows; t++)
	{
		...
		if (t == inputs.n_rows - 1)
		{
			...
			ps[t] = arma::exp(ys[t]) / arma::sum(arma::exp(ys[t]));
			// (3,1)
	...
	dWxh = zeros<mat>(this->Wxh.n_rows, this->Wxh.n_cols);
	dWhh = zeros<mat>(this->Whh.n_rows, this->Whh.n_cols);
	dWhy = zeros<mat>(this->Why.n_rows, this->Why.n_cols);
	dbh = zeros<mat>(this->bh.n_rows, this->bh.n_cols);
	dby = zeros<mat>(this->by.n_rows, this->by.n_cols);
	dhnext = zeros<mat>(hs[0].n_rows, hs[0].n_cols);
...
mat RNN::score2onehot(double score)
{
	...
	mat zs = zeros<mat>(this->n_output_classes, 1);
	zs(pos_idx, 0) = 1;
