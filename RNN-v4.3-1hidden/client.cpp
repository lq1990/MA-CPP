...
void train_rnn()
{
	...
	for (double lambda = 0., i = 0; lambda <= maxLambda; lambda+= intervalLambda, i++)
	{
		...
		rnn.initParams();
		// 1. 改变lambda，会得到不同的参数
		rnn.trainMultiThread(listStructTrain, opt, RNN::n_threads, lambda); // train RNN
		rnn.saveParams();
		...
		calcPredLossMeanAccu("listStructCV", cvLossMean, cvAccu); // 计算 CV数据集的 lossMean
		double trainLossMean=0., trainAccu=0.;
		calcPredLossMeanAccu("listStructTrain", trainLossMean, trainAccu); // 计算 Train数据集的 lossMean
